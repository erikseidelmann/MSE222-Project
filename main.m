...
y_top = 0.30; %Initial height of ball with spring
x_right = 0.30; %Initial horizontal position of ball with spring
m_ball= 0.006; % Mass of Ball
r_ball = 0.00815; %Radius of Ball
I_ball = 2/5*m_ball*r_ball^2; %Moment of inertia of ball
g = 9.81; %gravity of the Earth
...
omega(index_enters_ramp+1) = omega(index_enters_ramp);
...
omega(index_projectile_start+1) = omega(index_projectile_start);
...
a_x = 0.8*g*sind(25)*cosd(25); %0.8 is a drag factor
...
omega(index_proj_start+1) = omega(index_proj_start);
...
omega(index_pipe_bottom_start+1) = omega(index_pipe_bottom_start);
...
omega(index_proj_bb+1) = omega(index_proj_bb);
...
%comet(1000*x_ball(1800:length(x_ball)),1000*y_ball(1800:length(y_ball)),0)
data = [time, x_ball, y_ball, vx_ball, vy_ball, omega]
