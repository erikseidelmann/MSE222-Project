...
for i=2:length(x_spring)
    ...
    ay_ball(i,1) = 0;
    alpha(i,1) = 0;
    Nx(i,1) = -a_spring(i-1)*m_ball;
    Ny(i,1) = -g*m_ball;
    N(i,1) = sqrt(Ny(i)^2 + Nx(i)^2);
end
...
d_curve = 0.1114;
r_curve = d_curve/2;
N(index_curve_enter) = omega(index_curve_enter)^2*r_curve;
%Solve curve ODE
...
for k = 2:length(theta)%index_curve_enter+1:length(time)
    ...
    %N(k) = ;
    N = vertcat(N, omega_ball_curve(k)^2*r_ball);
...
N(index_leave_curve+1) = N(index_leave_curve);
...
N(index_enters_ramp+1) = N(index_enters_ramp);
...
while x_ball(l-1) > 0.0985 %0.17-0.018-56.61cos19.1 is ideal
    ...
    N(l) = sqrt(((a_y*m_ball) + m_ball*g)^2 + (a_x*m_ball)^2);
    l = l+1;
end
...
time(index_metal_start+1) = time(index_metal_start) + 0.001;
%N(index_metal_start+1) = sqrt(x_impulse(index_metal_start+1,1)^2 + y_impulse(index_metal_start+1,1)^2)/(time(index_metal_start+1)-time(index_metal_start));
N(index_metal_start+1) = 0;
...
omega(index_pipe_top_start+1) = 7/4*omega(index_pipe_top_start);
vx_ball(index_pipe_top_start+1) = -omega(index_pipe_top_start+1)*r_ball*(4/7)^2;
vy_ball(index_pipe_top_start+1) = 0;
N(index_pipe_top_start+1) = N(index_pipe_top_start);
...
t_step = 0.001;
N(index_slope_death_start+1) = 0;
x_impulse (index_slope_death_start+1,1) = m_ball*(vx_ball(index_slope_death_start+1)-vx_ball(index_slope_death_start));
y_impulse(index_slope_death_start+1,1)= m_ball*(vy_ball(index_slope_death_start+1)-vy_ball(index_slope_death_start));
angular_impulse(index_slope_death_start+1,1) = I_ball*(omega(index_slope_death_start+1)-omega(index_slope_death_start));
time(index_slope_death_start+1) = time(index_slope_death_start) + 0.001;
...
N(index_pipe_bottom_start+1) = -g*m_ball;
...
N(index_bb_start+1) = N(index_bb_start);
...
while theta_lg(l-1) < tol_angle*pi/180
    ...
    N(l) = 0;  % big assumption here that there is no force on the small rotator
    l = l+1;
end
%% Ramp into the train
...
N(index_slope_lg_start+1) = sqrt(((a_y*m_ball) + m_ball*g)^2 + (a_x*m_ball)^2);
t_step = 0.001;
...
N(index_projectile_lg_start+1) = 0;
%Zero acceleration in the x direction for projectile motion
a_x = 0;
a_y = -g;
t_step = 0.001;
...
a_x = g*sind(angle_cart)*cosd(angle_cart); %0.8 is a drag factor
a_y = -g*sind(angle_cart)*sind(angle_cart);
N(index_withcart_start+1) = sqrt(((a_y*m_ball) + m_ball*g)^2 + (a_x*m_ball)^2);
t_step = 0.001;
...
N(index_withcart_start+1) = -g*m_ball;
l = index_withcart_start+2;
t_step = 0.001;
